...
import { ORDER_STATUS, PAYMENT_STATUS, STORE_STATUS } from '../../../types/status'
import { MerchantSidebar } from '../MerchantSidebar'
import { StoreSwitcher } from '../StoreSwitcher'
import { useActiveStore } from '../store-context'
import { ATTENTION_ITEMS, DASHBOARD_METRICS, RECENT_ORDERS } from './mock-data'
...
/**
 * اللوحة تجيب الأسئلة الأربعة بالترتيب (الاتجاه الفني §9):
 * ماذا تغيّر؟ (شريط المؤشرات) ← ماذا يحتاج انتباهي؟ (نمط الانتباه) ←
 * ماذا أفعل الآن؟ (أفعال الرأس) ← كيف الأداء؟ (طلبات حديثة بعرض كامل).
 */
export function MerchantDashboardOverview() {
  ...
  const attention = seeded
    ? ATTENTION_ITEMS.map((entry) => ({
        id: entry.id,
        severity: entry.severity,
        title: entry.title,
        why: entry.why,
        actionLabel: entry.actionLabel,
        onAction: () => navigate(to(entry.to)),
      }))
    : []
  ...
  return (
    ...
    >
      ...
      {/* ═══ ماذا يحتاج انتباهي؟ ═══ */}
      <section className="dash-section" aria-labelledby="dash-attn-title">
        <h2 id="dash-attn-title">يحتاج انتباهك</h2>
        {loading ? (
          <Skeleton variant="rect" height={168} />
        ) : (
          <AttentionList items={attention} emptyDescription="لا مخزون ناقص ولا طلبات معلّقة في هذا المتجر." />
        )}
      </section>

      {/* ═══ كيف الأداء؟ — بعرض كامل، لا جدول مقصوص ═══ */}
